...
(function($) {
    ...
    DragonGrid.prototype = {
        ...
        init: function() {            
            if (this.$element.is('table')) {
                ...
                console.log('dragonGrid: source element must be a <table>')
            }
        },
        ...
        defineCols: function() {
            ...
            if (!that.options.cols.length) {
                for (var prop in that.source[0]) {
                    that.options.cols.push({
                        field: prop
                    });
                }
            }
        ...
        sorter: function(field, dir) {
            ...
            if (that.options.source) {
                ...
                that.source.sort(that.localSorter(field, dir));
                that.renderBody();
            }
        },
    ...
    //  Define the plugin
    //
    $.fn.dragonGrid = function (option) {
        return this.each(function () {
            var $this = $(this),
                data = $this.data('dragonGrid'),
                options = typeof option === 'object' && option;

            if (!data) {
                $this.data('dragonGrid', (data = new DragonGrid(this, options)));
            }
